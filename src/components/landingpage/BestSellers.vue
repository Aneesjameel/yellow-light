    ...
    <section class=" p-10">
      ...
      <div class="grid md:grid-cols-4 gap-6 mt-10 justify-center md:justify-start">
        <div v-for="product in products" :key="product.id" class="text-center">
          <img :src=" product.img" alt="Product Image" class=" h-[376px] w-[276px] object-cover mb-2 rounded-tr-2xl">
          <h3 class="text-lg text-left">{{ product.name }}</h3>
          <p class=" text-left font-bold">{{ product.price }}</p>
          <div class="flex gap-2">
            <div class=" text-left font-bold bg-yellow-600 text-white px-2 py-2 flex items-center text-sm mt-2">{{ product.a }}</div>
          <div class=" text-left font-bold bg-blue text-white px-5 py-2 flex items-center text-sm mt-2"><img src="/car.png" class="mr-3"/>{{ product.b }}</div>
          </div>
